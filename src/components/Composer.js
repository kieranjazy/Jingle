...
function Composer() {
    ...
    this.loadedInstruments = [{ "name": "01_synth_lead", "isLoaded": true, "type": "wavetable" }];
                ...
                for (let j = 0; j < this.activeNotes.length; j++) {
                    if (this.loadedInstruments[j]["type"] == "wavetable") {
                        for (let k = 0; k < this.activeNotes[j].length; k++) {
                            ...
                            this.keyPositions[j][this.activeNotes[j][k]] = (this.keyPositions[j][this.activeNotes[j][k]] + (this.keyfreqs[this.activeNotes[j][k]] * offset) % this.instrumentBank.wavetable.length); //changed [j] to .wavetable
                            let lower = Math.floor(this.keyPositions[j][this.activeNotes[j][k]]);
                            let upper = Math.floor(this.keyPositions[j][this.activeNotes[j][k]] + 0.5) % this.instrumentBank.wavetable.length;
                            ...
                            /*	Breakdown:
                                ...
                                */
                            nowBuffering[i + offset] = Math.min(Math.max(this.instrumentVolumes[j] * this.masterVolume * (this.instrumentBank.wavetable[lower] + ((this.instrumentBank.wavetable[upper] - this.instrumentBank.wavetable[lower]) * (this.keyPositions[j][this.activeNotes[j][k]] % 1))), -1), 1);
                            ...
                            this.keyPositions[j][this.activeNotes[j][k]] = (this.keyPositions[j][this.activeNotes[j][k]] + (this.keyfreqs[this.activeNotes[j][k]] * (totalNotes - offset)) % this.instrumentBank.wavetable.length);
                            offset++;
                        }
                    } else {
                        //Drum Kit (and Vox if there's time :) )
...
Composer.prototype.__record = async function (trackNum) {
    ...
    for (let i = 0; i < totalTrackLength; i++) {
        ...
        let currNotes = this.activeNotes.slice();
        let nextNotes = [];
