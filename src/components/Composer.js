			...
			for (let j = 0; j < this.activeNotes.length; j++) {
				if(this.loadedInstruments[j]["type"] == "wavetable") {
					for(let k = 0; k < this.activeNotes[j].length; k++) {
						this.keyPositions[j][this.activeNotes[j][k]] = (this.keyPositions[j][this.activeNotes[j][k]] + (this.keyfreqs[this.activeNotes[j][k]] * totalNotes)) % this.instrumentBank[j].length;
...
Composer.prototype.record = function(trackNum) { //Could record return how long it'll take the metronome to finish?
	if (trackNum < 0) {
