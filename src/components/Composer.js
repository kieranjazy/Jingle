import wobbly from '../wobbly.json'
import tr909 from '../TR_909.json'
import tr808 from '../TR_808.json'
import synthLead from '../synth_lead.json'
import enya from '../enya.json'
import choiraah from '../choiraah.json'
...
function Composer() {
	this.instrumentManifests = [
		{"name":"synth_lead","isLoaded":true,"type":0},
		{"name":"enya","isLoaded":true,"type":0},
		{"name":"wobbly","isLoaded":true,"type":0},
		{"name":"choiraah","isLoaded":true,"type":0},
		{"name":"TR_909","isLoaded":true,"type":1},
		{"name":"TR_808","isLoaded":true, "type":1}
	]
	...
	this.instrumentBank = [];
	this.storedInstruments = [synthLead.audiodata, enya.audiodata, wobbly.audiodata, choiraah.audiodata, tr909.audiodata, tr808.audiodata];
	this.bps = 1;
...
/*
Composer.prototype.setTrackInstrument = function (trackNo, instrumentData, manifestData) {
	if (this.loadedInstruments.length > trackNo && trackNo >= 0) {
		this.loadedInstruments.splice(trackNo, 1, manifestData);
	} else if (this.loadedInstruments.length == trackNo) {
		this.loadedInstruments.push(manifestData);
	}
	this.numTracks = this.loadedInstruments.length;
	this.fetchInstrument(trackNo, instrumentData);
}

Composer.prototype.fetchInstrument = function (trackNo, instrumentData) {
	let temp = JSON.parse(JSON.stringify(instrumentData));
	if(typeof(temp.audiodata) == "undefined") {
		console.log("Could not add instrument!");
	}
	if(this.instrumentBank.length == trackNo) {
		//this.instrumentBank.push(Array(temp.audiodata).slice());
		this.instrumentBank.push(temp)
	} else if(trackNo < this.instrumentBank.length && trackNo >= 0)
		//this.instrumentBank[trackNo] = Array(temp.audiodata).slice();
		this.instrumentBank[trackNo] = temp
};
*/

Composer.prototype.setTrackInstrument = function (trackNo, instrumentPosition) {
	if (this.loadedInstruments.length > trackNo && trackNo >= 0) {
		this.loadedInstruments[trackNo] = this.instrumentManifests[instrumentPosition];
	} else if (this.loadedInstruments.length == trackNo) {
		this.loadedInstruments.push(this.instrumentManifests[instrumentPosition]);
		this.numTracks = this.loadedInstruments.length;
	} else {
		return;
	}
	this.fetchInstrument(trackNo, instrumentPosition)
}

Composer.prototype.fetchInstrument = function (trackNo, instrumentData) {
	let temp = this.storedInstruments[instrumentData];
	if(this.instrumentBank.length == trackNo) {
		this.instrumentBank.push(temp);
		this.numTracks = this.instrumentBank.length;
	} else if(trackNo < this.instrumentBank.length && trackNo >= 0)
		this.instrumentBank[trackNo] = temp;
};
